...
$w_k \in D$ is the random disturbance at time $k$. $U$ and $D$ are finite sets.
The probability that the disturbance takes the value $d_j \in D$ at time $k$ is, $\mathbb{P}[w_k = d_j] = p_j$, 
...
For example, if the system is a pond, then $x_k\geq 0$ may be the water level of the pond at time $k$,
...
WeThe goal of this paper is to compute the sets of initial states from which the risk of extreme constraint violations can be made small via a control policy, where risk is quantified
using Conditional Value-at-Risk.

the set of initial conditions from which there is small risk of large constraint violations over the finite time horizon.



design an algorithm that computes a \textit{risk-sensitive safe set} 
