...
% \begin{figure}[thpb]
      % \centering
      % \includegraphics[scale=0.5]{dyn_prog_J0_sept112018.jpg}
      % \caption{$J_0(x,\alpha)$ versus $(x, \alpha) \in G_s \times G_c$ generated by our dynamic programming algorithm~\eqref{bell} is shown.
	  % $J_0(x,\alpha)$ is our best estimate of~\eqref{J0} with the stage cost, $c(x) = \beta e^{m g(x)}$.}
      % \label{J0dp}
% \end{figure}
