...
The goal of this paper is to design an algorithm that computes \textit{risk-sensitive sets} 
for a system of the form specified in Sec.~\ref{model}.
A risk-sensitive set is, informally, the set of initial conditions of the system, from which 
the risk of large constraint violations over time can be made sufficiently small by an appropriate control policy.
...
The expression, $\text{CVaR}_\delta\left(X_y^\pi\right)$, means that the conditional value-at-risk is taken with respect to the probability distribution of $(w_0, \dots, w_{T-1})$.
For any $\delta \in (0,1)$ and any $r \in \mathbb{R}$ START HERE!, the \textit{risk-sensitive safe set} is defined by,
\begin{equation}
\mathcal{U}_{\delta,0} := \Big\{y \in S\text{ }|\text{ }\exists \pi \in \Pi \text{ such that } \text{CVaR}_\delta\left(X_y^\pi\right) < 0 \Big\},
\label{U0}\end{equation}
where the random variable, $X_y^\pi$, is defined in~\eqref{xpiy}.
More generally, for any $\delta \in (0,1)$, we can define the \textit{risk-sensitive $r$-sublevel set} as follows,
\begin{equation} \begin{aligned}
...
where $r \in \mathbb{R}$.
A \textit{risk-sensitive set} is shorthand for any \textit{risk-sensitive $r$-sublevel set}.
The problem addressed in this paper is how to compute~\eqref{Ur} for a finite collection of $r \in \mathbb{R}$.

\section{Properties of $\mathcal{U}_\delta$}\label{prop}
