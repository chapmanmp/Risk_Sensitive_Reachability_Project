...
risk-sensitive safety guarantees are closely related, if the risk measure is CVaR, as we shall explain in Sec.~\ref{prop}.
...
The goal of this paper is to design an algorithm that computes risk-sensitive sets 
for a system of the form specified in Sec.~\ref{model}.
A \textit{risk-sensitive set} is, informally, the set of initial conditions of the system, from which 
the risk of large constraint violations over time can be made small by an admissible control policy.
%there is sufficiently small risk of large constraint violations over time.
...
where the meaning of large is determined by the value of $\delta$ (Fig.~\ref{cvar}).
...
We are now ready to define a risk-sensitive set formally.
...
starting from a given (non-random) state $y \in S$ at time zero.  
...
where $g$ satisfies~\eqref{g}, and the randomness in $X_y^\pi$ comes from the disturbance sequence, $(w_0, w_1, \dots, w_{T-1})$.
Given $\delta \in (0,1)$, the \textit{risk-sensitive $r$-sublevel set} is defined by,
\begin{equation} \begin{aligned}
\mathcal{U}_{\delta,r} := &  \Big\{y \in S\text{ }|\text{ }\exists \pi \in \Pi \text{ such that } \text{CVaR}_\delta\left(X_y^\pi\right) < r \Big\} \\
= & \Big\{y \in S \text{ }|\text{ } {\underset{\pi \in \Pi}\min}\text{ } \big\{\text{CVaR}_\delta\left(X_y^\pi\right)\big\} < r \Big\},
\label{Ur}\end{aligned}\end{equation}
where $r \in \mathbb{R}$, and the conditional value-at-risk 
is taken with respect to the probability distribution of $(w_0, w_1, \dots, w_{T-1})$.
[Take out?: We give the risk-sensitive zero-sublevel set a particular name: the \textit{risk-sensitive safe set}.]
In this paper, we seek to compute $\mathcal{U}_{\delta,r}$ for a finite collection of $(\delta, r) \in (0,1)\times \mathbb{R}$.

\section{Properties of Risk-Sensitive Sets}\label{prop}
Here we present key properties of risk-sensitive sets.
The first property is that every state in the risk-sensitive zero-sublevel set, $\mathcal{U}_{\delta,0}$, enjoys a probabilistic safety guarantee.
...
\end{lemma}
\begin{proof}
Suppose $\text{CVaR}_\delta(Z) < 0$. \\
$\exists c \in \mathbb{R}$ such that $c + \frac{1}{\delta}\mathbb{E}\big[\max\{Z-c,0\}\big] < 0$, by~\eqref{cvar}.\\
But, $\frac{1}{\delta}\mathbb{E}\big[\max\{Z-c,0\}\big] \geq 0$, so $c < 0$.\\
Thus, $\exists c<0$ such that $c + \frac{1}{\delta}\mathbb{E}\big[\max\{Z-c,0\}\big] < 0$.\\
Set $\alpha := -c$.\\
Then, $\exists \alpha>0$ such that
\begin{equation}\begin{aligned}
-\alpha + \frac{1}{\delta}\mathbb{E}\big[\max\{Z+\alpha,0\}\big] & < 0 && \iff\\
\mathbb{E}\big[\max\{Z+\alpha,0\}\big] & < \delta\alpha && \iff \\
\frac{1}{\alpha}\mathbb{E}\big[\max\{Z+\alpha,0\}\big] & < \delta.
\end{aligned}\end{equation}
Since $\max\{Z+\alpha,0\}$ is a non-negative random variable, we can use Markov's Inequality, 
\begin{equation}
\mathbb{P}[\max\{Z+\alpha,0\}\geq \alpha]\leq \frac{1}{\alpha}\mathbb{E}\big[\max\{Z+\alpha,0\}\big].
\end{equation}
Thus, $\exists \alpha>0$ such that $\mathbb{P}[\max\{Z+\alpha,0\}\geq \alpha] < \delta$.\\
If $\mathbb{P}[\max\{Z+\alpha,0\}\geq \alpha] = \mathbb{P}[Z \geq 0]$, then $\mathbb{P}[Z \geq 0]<\delta$, which would complete the proof.
We now prove this sufficient condition:
\begin{equation}\begin{aligned}
\mathbb{P}&[\max\{Z+\alpha,0\}\geq \alpha]\\
&=\mathbb{P}\left[\left\{Z+\alpha\geq \alpha \cap Z+\alpha\geq 0\right\} \cup \left\{0 \geq \alpha \cap Z+\alpha < 0\right\}\right] \\
&=\mathbb{P}\left[\{Z+\alpha\geq \alpha\} \cap \{Z+\alpha\geq 0\}\right] + \mathbb{P}\left[\{0 \geq \alpha\} \cap \{Z+\alpha < 0\}\right] \\
&=\mathbb{P}[\{Z+\alpha\geq \alpha\} \cap \{Z+\alpha\geq 0\}] \\
&=\mathbb{P}[Z\geq0].
\end{aligned}\end{equation}
The first equality holds by the definition of $\max\{Z+\alpha,0\}$. 
The second equality holds since the probability of a disjoint union of events
is the sum of the probabilities of the individual events.
The third and fourth equalities hold because $\alpha > 0$; 
$\{Z+\alpha\geq \alpha\} \cap \{Z+\alpha\geq 0\}$ is equivalent to $\{Z+\alpha\geq \alpha\}$, which is equivalent to $\{Z\geq 0\}$.
\end{proof}
...
The next result uses Lemma~\ref{lemma1} to indicate that every state in $\mathcal{U}_{\delta,0}$ enjoys a probabilistic safety guarantee.
\begin{corollary}
$\mathcal{U}_{\delta,0}$, as defined in~\eqref{Ur} for $r=0$, is a subset of $\mathcal{S}_\delta$,
\begin{equation}
