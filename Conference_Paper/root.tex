...
\begin{equation*}\begin{aligned}
\text{CVaR}_\alpha \big[ Y_x^\pi \big] & < \beta e^{m\cdot r} && \iff \\
\text{CVaR}_\alpha \big[ Y_x^\pi /\beta \big] & < e^{m\cdot r} && \iff \\
\frac{1}{m}\log \left( \text{CVaR}_\alpha \big[  Y_x^\pi /\beta \big] \right) & < r && \implies \\
...
& k = N-1, \dots, 0,
\end{aligned}\end{equation}
with the initialization, $J_N(x_k, y_k) := c(x_k)$. 
Then, $J_0^*(x,\alpha)$, as defined in~\eqref{J0}, is given by $J_0(x, \alpha)$, 
the function at the last step of the recursion evaluated at $(x,\alpha)$.
\end{theorem}
\begin{remark}
the functions J's are well-defined and finite because D is a finite set
$x_{k+1}$ is given by~\eqref{sys}, and $\mathcal{R}(y_k, \mathbb{P})$ is defined in~\eqref{decomp}
We choose $c(x) := $
\end{remark}
\begin{remark}
Chow proposed bellman operator (infinite time/discounted case). no switch min/max required in ours.
\end{remark}
\begin{remark}
we do not show that policy is optimal...exploring the techniques...
\end{remark}
