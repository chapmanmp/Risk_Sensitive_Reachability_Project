...
%and prove the correctness of the algorithm for finite probability spaces. Our proof is a novel contribution, as it does not require the assumption of strong duality, which was required in a previous paper.
...
%~\cite{serraino2013conditional} says "Cvar protects against outcomes that hurt the most", do i need citation in the sentence above? Kevin says no need
...
Sec.~\ref{sec::alg} presents a value-iteration algorithm that computes \textcolor{red}{approximations} for risk-sensitive safe sets. %include proof?
...
\footnotetext{See ~\cite{abate2008probabilistic}, Equations 11 and 13.
Note that ref.~\cite{abate2008probabilistic}  considers hybrid dynamic systems that evolve under Markov policies. }
...
%\begin{equation*}\begin{aligned}
%C(R, Z&; x_k, y_k, \pi_k) = \\
%& \textstyle\sum_{j=1}^W v_j \cdot \text{CVaR}_{y_k v_j}[ Z | x_{k+1}^j, \pi_{k+1} ] \cdot \mathbb{P}[w_k = d_j],\\
%\end{aligned}\end{equation*}
%
%where $x_{k+1}^j = f(x_k, \mu_k(x_k, y_k), d_j)$, and $v_j = R(d_j)$. %In words, $x_{k+1}^j$ is the $j$\textsuperscript{th} realized state at time $k+1$, 
%given the control at time $k$, $u_k = \mu_k(x_k, y_k)$, and the $j$\textsuperscript{th} realized disturbance at time $k$, $w_k = d_j$.
%Also, $v_j$ is the $j$\textsuperscript{th} realization of the random variable, $R \in \mathcal{R}(x_k, y_k, \mu_k, \mathbb{P})$. 
%\end{remark}
%
\begin{remark}
...
\footnotetext{For $a\in \mathbb{R}$, $\text{CVaR}_\alpha[a+Z] = a + \text{CVaR}_\alpha[Z]$; see~\cite{kisiala2015conditional}, Sec. 2.2.}. \textcolor{red}{The last equality (a) is the core of our conjecture, as we need to justify why the worst-case density $R$ that attains the maximum is equal to the a priori chosen density $\bar{R}$ that defines the dynamics of the confidence level.}
